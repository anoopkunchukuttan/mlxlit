...
class Model():
	def __init__(self,mapping,embedding_size,max_sequence_length):
		...
		self.vocab_sizes = self.mapping.get_vocab_sizes()

		# Finds 41 length vector phonetic representation for each character of each language
		self.phonetic_vectors = self.mapping.get_phonetic_vectors()
		
		# Converting the character representation to embedding_size vector
		max_val = 0.1
		self.embed_W0 = tf.Variable(tf.random_uniform([41,embedding_size], -1*max_val, max_val), name = 'embed_W0') 
		self.embed_W = dict()
		for lang in self.lang_list:
			self.embed_W[lang] = tf.matmul(tf.constant(self.phonetic_vectors[lang], dtype = tf.float32), self.embed_W0)
		...
		self.out_b = dict()
		for lang in self.lang_list:
			self.out_W[lang] = tf.Variable(tf.random_uniform([self.rnn_size,self.vocab_sizes[lang]], -1*max_val, max_val))
			self.out_b[lang] = tf.Variable(tf.constant(0., shape = [self.vocab_sizes[lang]]))
	...
	def seq_loss(self, target_sequence, target_masks, lang, initial_state):
		...
		for i in range(self.max_sequence_length):
			...
			onehot_labels = tf.sparse_to_dense(concated,tf.pack([batch_size,self.vocab_sizes[lang]]),1.0,0.0)
