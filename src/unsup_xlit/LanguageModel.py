...
class LanguageModel():
    ...
    def __init__(self,lang, mapping, representation,
        ...
        ####### Input embeddings 
        self.bitvector_embeddings = tf.constant(self.mapping.get_bitvector_embeddings(self.lang,self.representation),dtype=tf.float32)
        ...
        self.Wmat = tf.Variable(tf.random_uniform([self.bitvector_embedding_size, self.embedding_size], -1*max_val, max_val), name = 'lm_Wmat')
    ...
    def logit_next_char(self,current_input,prev_state): 
        ...
        current_embeddings = tf.add(tf.nn.embedding_lookup(self.embed_W,current_input),self.embed_b)
...
if __name__ == '__main__' :
    ...
    parser.add_argument('--data_dir', type = str, help = """input directory contaning three files. (train,valid,test).txt. 
    ...
    parser.add_argument('--use_mapping', type = str, default = None, help = 'If you want to use an existing vocabulary, provide the path to the mapping file')

    parser.add_argument('--embedding_size', type = int, default = 256, help = 'size of character representation')
    parser.add_argument('--rnn_size', type = int, default = 512, help = 'size of output of encoder RNN')
    ...
    parser.add_argument('--mapping_class', type = str, default = 'IndicPhoneticMapping',  help = 'class to be used for mapping. Possible values: IndicPhoneticMapping, CharacterMapping')
    ...
    train_data = MonoDataReader.MonoDataReader(args.lang, args.data_dir+'/train.txt', mapping,args.max_seq_length)
    ...
    valid_data = MonoDataReader.MonoDataReader(args.lang,args.data_dir+'/valid.txt',mapping,args.max_seq_length)
    ## Reading test data 
    test_data  = MonoDataReader.MonoDataReader(args.lang,args.data_dir+'/test.txt' ,mapping,args.max_seq_length)
