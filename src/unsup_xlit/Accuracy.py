...
target_sequences = map(lambda x: x.strip().split(), codecs.open(sys.argv[1],'r','utf-8').readlines())
num_words = len(target_sequences)

# Trim predicted sequences so as to match size with the actual words
predicted_sequences = map(lambda x: x.strip().split(), codecs.open(sys.argv[2],'r','utf-8').readlines())
predicted_sequences_trimmed = [predicted_sequences[i][1:predicted_sequences[i].index(u'EOW')] for i in range(num_words)]
